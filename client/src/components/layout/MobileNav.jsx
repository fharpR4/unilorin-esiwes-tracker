...
import {
  ...
  CalendarCheck, FolderGit2, Bell, Settings,
} from 'lucide-react';
...
const MOBILE_NAV = {
  [ROLES.STUDENT]: [
    ...
    { href: '/projects/mine', icon: FolderGit2, label: 'Project' },
    ...
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-30 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700">
      <div className="flex items-center justify-around py-1.5">
        {items.map(({ href, icon: Icon, label }) => {
          ...
          return (
            <Link key={href} href={href} className="flex flex-col items-center gap-0.5 px-3 py-1.5 relative">
              <div className="relative">
                ...
                {isNotif && unreadCount > 0 && (
                  <span className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full bg-red-500 text-white text-xs font-bold flex items-center justify-center" style={{ fontSize: 8 }}>
                    {unreadCount > 9 ? '9' : unreadCount}
                  </span>
                )}
              </div>
              <span className={cn('text-xs font-medium transition-colors', isActive ? 'text-unilorin-primary dark:text-blue-400' : 'text-gray-400 dark:text-gray-500')}>
                {label}
              </span>
            </Link>
          );
