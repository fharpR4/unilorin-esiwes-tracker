...
import { Users, Search, ChevronRight, Hash } from 'lucide-react';
...
export default function StudentsPage() {
  ...
  const { data, isLoading } = useApi(
    `/users/students${search ? `?search=${encodeURIComponent(search)}` : ''}`
  );
    ...
    <>
      ...
      <AppLayout pageTitle="All Students" allowedRoles={[ROLES.COORDINATOR, ROLES.ADMIN]}>
        <div className="space-y-4">
          ...
          {!isLoading && total > 0 && (
            <p className="text-xs text-gray-400">{total} student{total !== 1 ? 's' : ''} found</p>
          )}
          ...
          {isLoading ? <LoadingSpinner /> : students.length === 0 ? (
            <EmptyState
              ...
              description={search ? `No results for "${search}"` : 'No students registered yet.'}
            />
          ) : (
                ...
                <Link key={student._id} href={`/students/${student._id}`}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-0.5 text-xs text-gray-400 flex-wrap">
                      ...
                      {student.level && <span>· {student.level} Level</span>}
                    </div>
                  </div>
