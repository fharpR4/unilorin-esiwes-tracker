...
const getAllUsers = asyncHandler(async (req, res) => {
  ...
  res.status(200).json({ success: true, count: total, page: parseInt(page), pages: Math.ceil(total / parseInt(limit)), data: { users } });
});
...
const updateUser = asyncHandler(async (req, res, next) => {
  const user = await User.findById(req.params.id);
  if (!user) return next(new ApiError(404, 'User not found.'));
  const allowed = ['firstName','lastName','phone','courseOfStudy','matricNumber','department','level','institution','faculty','supervisorDepartment','supervisorInstitution','staffId','title'];
...
const deactivateUser = asyncHandler(async (req, res, next) => {
  const user = await User.findById(req.params.id);
  if (!user) return next(new ApiError(404, 'User not found.'));
  if (user._id.toString() === req.user._id.toString()) return next(new ApiError(400, 'You cannot deactivate your own account.'));
  user.isActive = false;
  await user.save();
...
const getStudents = asyncHandler(async (req, res) => {
  ...
  res.status(200).json({ success: true, count: total, page: parseInt(page), pages: Math.ceil(total / parseInt(limit)), data: { students } });
});
...
const getStudentsForSupervisor = asyncHandler(async (req, res) => {
  ...
  if (institution) {
    ...
    const isObjectId = /^[a-f\d]{24}$/i.test(institution);
    let inst = null;
    if (isObjectId) {
      inst = await Institution.findById(institution);
    } else {
...
// Works both with and without auth (used on registration page too)
const getSupervisors = asyncHandler(async (req, res) => {
  ...
  const supervisors = await User.find(filter)
    .select('firstName lastName email phone faculty supervisorDepartment supervisorInstitution staffId title')
    .sort({ lastName: 1, firstName: 1 });
...
const getMyAssignedStudents = asyncHandler(async (req, res) => {
  ...
  const onlyManual = manualAssigned.filter((as) => as.student && !appStudentIds.has(as.student._id.toString()));
...
const assignStudentToSelf = asyncHandler(async (req, res, next) => {
  ...
  const already = supervisor.assignedStudents.some((a) => a.student.toString() === studentId.toString());
  if (!already) {
  ...
  await createNotification({
    ...
    message: `${supervisor.title ? supervisor.title + ' ' : ''}${supervisor.firstName} ${supervisor.lastName} has added you to their student list.`,
    link: '/applications',
  });
